"""Generate notebooks/kaggle_phase3_concept_bottleneck.ipynb.

A standalone Kaggle notebook that demonstrates a Concept Bottleneck
generator: rather than learning free-text radiology English end-to-end (the
Phase-2 BART approach), the model only learns to predict ~9 VASARI features
from the 3D MRI. The fluent report is then assembled deterministically from
those predicted features, so every claim is auditable and free-text
hallucination is structurally impossible.

Architecture (per-feature classifier heads on a small shared encoder):

  [BraTS volume 4×64×64×64]
        ↓ small 3D CNN encoder (~70K params)
  [B, 128] pooled features
        ↓ 9 small linear heads
  {tumor_present, side, region, enhancement, edema, necrosis,
   multifocal, crosses_midline, size_cm}
        ↓ deterministic VASARI template (already in our package)
  Free-text radiology report
        ↓ NeuroVal-3D
  Per-axis validator scores

Compared to Phase-2 BART: 143M → ~280K parameters (≈500× smaller),
zero hallucination by construction, every output trace-able to a
specific learned feature.

Run order: Setup → Labels → Model → Train → Inference → Compare to BART → Save.
...
NB_PATH = Path("notebooks/kaggle_phase3_concept_bottleneck.ipynb")
