"""Render docs/REVIEW_2_REPORT.md -> docs/REVIEW_2_REPORT.pdf.

Same proven pipeline as `generate_phase_summary_pdf.py`. Every `## ` heading
starts on a new page, top-level `# ` headings break to a new page as well so
the three review parts (System Design / Implementation / Testing) each get
their own clean section opening.
"""
...
SRC = ROOT / "docs" / "REVIEW_2_REPORT.md"
DST = ROOT / "docs" / "REVIEW_2_REPORT.pdf"
...
TITLE = ParagraphStyle(
    "Title", parent=styles["Title"], fontSize=28, leading=34,
    textColor=colors.HexColor("#1f3a4f"), alignment=1, spaceAfter=14,
)
...
H1 = ParagraphStyle(
    ...
    textColor=colors.HexColor("#1f3a4f"), spaceBefore=0, spaceAfter=14,
    keepWithNext=True,
)
H2 = ParagraphStyle(
...
CODE_BLOCK = ParagraphStyle(
    "CodeBlock", parent=BODY, fontName="Courier", fontSize=9, leading=12,
    leftIndent=14, rightIndent=14, spaceBefore=6, spaceAfter=10,
    backColor=colors.HexColor("#f5f5f5"), textColor=colors.HexColor("#1d1d1d"),
    borderColor=colors.HexColor("#dddddd"), borderWidth=0.5, borderPadding=8,
)
...
def parse_markdown(md: str) -> list:
    ...
    # Main body — H1 / H2 always start on a new page; H3 inline
    while i < len(lines):
        line = lines[i]

        # Horizontal rule -> spacing only (page break is on the next H1/H2)
        if line.strip() == "---":
            flow.append(Spacer(1, 0.05 * inch))
            i += 1
            continue

        # Headings — only H1 (# ) starts a new page; H2 (## ) flows inline
        if line.startswith("### "):
        ...
        if line.lstrip().startswith("```"):
            ...
            if i < len(lines):
                i += 1  # skip closing ```
            code_text = "<br/>".join(_escape_code(c) for c in code_lines)
            flow.append(Paragraph(code_text, CODE_BLOCK))
            continue
        ...
        if line.startswith("- ") or line.startswith("* "):
            while i < len(lines) and (lines[i].startswith("- ") or lines[i].startswith("* ")
                ...
                if re.match(r"^   +(- |\* )", lines[i]):
                    ...
                    flow.append(Paragraph("o " + _inline(content), sub_style))
                else:
...
def _render_table(tbl_lines: list[str]) -> Table:
    ...
    tbl.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#1f3a4f")),
        ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
        ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
        ("VALIGN", (0, 0), (-1, -1), "TOP"),
        ("ROWBACKGROUNDS", (0, 1), (-1, -1),
         [colors.HexColor("#f7faf9"), colors.white]),
        ("LINEBELOW", (0, 0), (-1, 0), 0.5, colors.HexColor("#1f3a4f")),
...
def main() -> None:
    ...
    doc = SimpleDocTemplate(
        ...
        title="NeuroVal-3D - Minor Project Review 2",
    ...
    def add_page_furniture(canvas, doc):
        ...
        canvas.drawString(
            0.85 * inch, 0.55 * inch,
            "NeuroVal-3D - Minor Project Review 2 - May 2026",
        )
